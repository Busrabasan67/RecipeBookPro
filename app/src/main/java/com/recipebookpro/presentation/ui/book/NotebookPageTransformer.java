...
public class NotebookPageTransformer implements ViewPager2.PageTransformer {
    ...
    public void transformPage(@NonNull View page, float position) {
        ...
        if (position < -1) { 
            page.setAlpha(0f);
        } else if (position <= 0) { // Sola doğru çevrilen mevcut sayfa | Current page turning left
            page.setAlpha(1f);
            
            page.setTranslationX(pageWidth * -position);
            
            // Sol kenar (cilt) pivot | Left edge (spine) pivot
            page.setPivotX(0); 
            page.setPivotY(page.getHeight() / 2f);
            
            // 0 ile -180 derece arasında döndür | Rotate between 0 and -180 degrees
            ...
            // Foreshortening for paper curl illusion. This prevents the page from rotating like a flat board.
        ...
        } else if (position <= 1) { // Alttan gelen sonraki sayfa | Next page coming from underneath
            page.setAlpha(1f);
            
            page.setTranslationX(pageWidth * -position); 
            page.setRotationY(0); // Alttaki sayfa düz durur | Bottom page stays flat
            page.setScaleX(1f);
            page.setScaleY(1f);
